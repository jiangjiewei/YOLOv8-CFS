...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1.0, 1.0, 1.0])).cuda(args.gpu)
    # use_cuda = True
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.arch.find('alexnet') != -1:
            ...
            pre_name = './models_ori/slit_lamp/inception_v3'
        elif args.arch.find('densenet121') != -1:
            pre_name = './models_ori/mobile_w111_lr0.001_size128_epoch80/densenet121'
...
def fundus_test():
    ...
    for index_name in range(0,len(model_names)):
        args.arch = model_names[index_name]
        for i in range(0,len(dataset_subdir)):
            dataset_dir = '/data/jiangjiewei/dk/data/nocropped_slitlamp/val1'
            #resultset_dir = './result_slit_lamp/' + dataset_subdir[i]
            #resultset_dir = './result_by_ori_model/result_slit_lamp/'
            resultset_dir = '/data/jiangjiewei/dk_test/features/DenseNet121/val'
            args, model, val_transforms = load_modle_trained(args)
            mk_result_dir(args, resultset_dir)
            fundus_test_exec(args, model, val_transforms, dataset_dir,resultset_dir)
...
def load_modle_trained(args):
    ...
    PATH='/data/jiangjiewei/dk/models_ori/slit_lamp/densenet121_model_best.pth.tar'
...
def fundus_test_exec(args,model,val_transforms,testdata_dir='./data/val1',resultset_dir='./data/val1'):
    ...
    if 'inception' in args.arch:
        modules_0 = list(model.module.children())[:-7]
        modules_1 = list(model.module.children())[-6:-1]
    ...
    features_grade1 = desdatadir + '/' + 'features_normal.txt'
    features_grade2 = desdatadir + '/' + 'features_keratitis.txt'
    features_grade3 = desdatadir + '/' + 'features_other.txt'
    model.eval()
    ...
    with torch.no_grad():
        ...
        for img in img_list:
            ...
            img_features_cpu = img_features.cpu().numpy()
            # img_features_cpu = img_features_cpu.tolist()
            file_features_object = open(features_grade2, 'a')
            for features_index in img_features_cpu:
                file_features_object.writelines(str(features_index) + ' ')
            file_features_object.writelines('\n')
            prob = F.softmax(ouput, dim=1)
        ...
        for img in img_list:
            ...
            img_features_cpu = img_features.cpu().numpy()
            # img_features_cpu = img_features_cpu.tolist()
            file_features_object = open(features_grade3, 'a')
            for features_index in img_features_cpu:
                file_features_object.writelines(str(features_index) + ' ')
            file_features_object.writelines('\n')
            prob = F.softmax(ouput, dim=1)
